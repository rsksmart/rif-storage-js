...
function isNodeReadable (stream: unknown): stream is Readable {
  return (stream as Readable)[Symbol.asyncIterator] !== undefined
}

export default function normalizeStream (stream: Readable | ReadableStream<Uint8Array>): Readable {
  if (isNodeReadable(stream)) return stream
